...
def plotExplicitEulerErrors(x0, v0, h, N):
    (TT, XX, VV) = explicitEulerErrors(x0, v0, h, N)
    plotXVT(TT, XX, VV, 'Xnumeric(t)-Xanalytic(t) of spring', 'Vnumeric(t)-Vanalytic(t) of spring')


# Problem 3 ========================================

# It is slowly accumulating more options...
def plotXY(xX, yY, titleString, xlabel, ylabel, logLog=False, semiLogY=False, markPoints=[]):
...
# Using the w=1 case....
def eE_truncation(x0, v0, h0, time):
    h0 = float(h0)
    ...
    for i in range(5):
        ...
        truncError[i] = max(XX)
    plotXY(HH, truncError, "Truncation error of explicit Euler", "h used", "maximum truncation error in " + str(time) + " units of time")


# Problem 4 ========================================

def eE_energy(x0, v0, h, N):
    (TT, XX, VV) = explicitEuler(x0, v0, h, N)
    EE = XX*XX + VV*VV
    plotXY(TT, EE, "Energy x^2+v^2 of numerical explicit Euler", "time", " energy ")
...
def plotImplicitEuler(x0, v0, h, N):
...
def plotImplicitEulerErrors(x0, v0, h, N):
...
def iE_energy(x0, v0, h, N):
    (TT, XX, VV) = implicitEuler(x0, v0, h, N)
    EE = XX*XX + VV*VV
    plotXY(TT, EE, "Energy x^2+v^2 of numerical implicit Euler", "time", " energy ")
...
def eE_iE_phaseSpace(x0, v0, h, N):
...
def symp_energy(x0, v0, h, N):
    (TT, XX, VV) = symplecticEuler(x0, v0, h, N)
    EE = XX*XX + VV*VV
    plotXY(TT, EE, "Energy x^2+v^2 of numerical symplectic Euler", "time", " energy ")



# Problem 4 ========================================

def symp_lag(x0, v0, h, N):
